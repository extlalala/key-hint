...
// copied from com.intellij.openapi.editor.actions.NextPrevWordHandler

fun nextPrevWordRange(editor: Editor): IntRange {
	val caret = editor.caretModel.currentCaret
	return nextPrevWordOffset(editor, caret, false)..nextPrevWordOffset(editor, caret, true)
}

fun nextPrevWordOffset(editor: Editor, caret: Caret, isNext: Boolean): Int {
	if (EditorUtil.isPasswordEditor(editor))
		return if (isNext) editor.document.textLength else 0

	val currentPosition = caret.visualPosition
	if (caret.isAtBidiRunBoundary && (isNext xor currentPosition.leansRight))
		return editor.visualPositionToOffset(currentPosition.leanRight(!currentPosition.leansRight))

	return if (isNext xor caret.isAtRtlLocation) {
		nextWordOffset(editor, editor.settings.isCamelWords)
	} else {
		previousWordOffset(editor, editor.settings.isCamelWords)
	}
}

fun previousWordOffset(editor: Editor, camel: Boolean): Int {
	return previousCaretStopOffset(
		editor, EditorSettingsExternalizable.getInstance().caretStopOptions.backwardPolicy,
		camel
	)
}

fun previousCaretStopOffset(
	editor: Editor, caretStopPolicy: CaretStopPolicy,
	isCamel: Boolean
): Int {
	...
	val currentFoldRegion = editor.foldingModel.getCollapsedRegionAtOffset(offset - 1)
	if (currentFoldRegion != null) {
		newOffset = currentFoldRegion.startOffset
	} else {
		newOffset = EditorActionUtil.getPreviousCaretStopOffset(editor, caretStopPolicy, isCamel)
		if (newOffset == offset) return offset

		val foldRegion = editor.foldingModel.getCollapsedRegionAtOffset(newOffset)
		if (foldRegion != null && newOffset > foldRegion.startOffset) {
			newOffset = foldRegion.endOffset
		}
	}

	if (editor is EditorImpl) {
		val boundaryOffset = editor.findNearestDirectionBoundary(offset, false)
		if (boundaryOffset >= 0) {
			newOffset = max(boundaryOffset, newOffset)
		}
		return editor.logicalPositionToOffset(editor.offsetToLogicalPosition(newOffset).leanForward(true))
	} else {
		return newOffset
	}
}

fun nextWordOffset(editor: Editor, camel: Boolean): Int {
	return nextCaretStopOffset(
		editor, EditorSettingsExternalizable.getInstance().caretStopOptions.forwardPolicy,
		camel
	)
}

fun nextCaretStopOffset(
	editor: Editor, caretStopPolicy: CaretStopPolicy, isCamel: Boolean
): Int {
	...
	val currentFoldRegion = editor.foldingModel.getCollapsedRegionAtOffset(offset)
	if (currentFoldRegion != null) {
		...
		val foldRegion = editor.foldingModel.getCollapsedRegionAtOffset(newOffset)
		if (foldRegion != null) {
			newOffset = foldRegion.startOffset
		}
	}
	if (editor is EditorImpl) {
		val boundaryOffset = editor.findNearestDirectionBoundary(offset, true)
		if (boundaryOffset >= 0) {
			newOffset = min(boundaryOffset, newOffset)
		}
	}
	return newOffset
}
...
private fun calcTargetVisualLine(editor: Editor, visualLine: Int, yIncrement: Int): Int {
	val startY = editor.visualLineToY(visualLine) + (if (yIncrement > 0) editor.lineHeight - 1 else 0)
	val targetY = startY + yIncrement
	var targetVisualLine = editor.yToVisualLine(targetY)
	val targetVisualLineBase = editor.visualLineToY(targetVisualLine)
	if (targetY < targetVisualLineBase) {
		if (yIncrement < 0) targetVisualLine--
	} else if (targetY >= targetVisualLineBase + editor.lineHeight) {
		if (yIncrement > 0) targetVisualLine++
	}
	return targetVisualLine
}
